...
void adicionarItemNoInicioDaLista(Lista* lista, int valor){
    ListaItem* listaItem = malloc(sizeof(ListaItem));
    ...
    if (lista->primeiroItem == NULL)
        ...
        while (aux->proximoItem != NULL) aux = aux->proximoItem;
...
void removerPrimeiroItemDaLista(Lista* lista){
    if (lista->primeiroItem == NULL) return;
    ListaItem* aux = lista->primeiroItem;
...
int tamanhoDaLista(Lista* lista){
    if (lista->primeiroItem == NULL) return 0;
    int tamanho;
    ListaItem* aux = lista->primeiroItem;
    for (tamanho = 0; aux != NULL; tamanho++) aux = aux->proximoItem;
    return tamanho;
}

// Função para retornar o valor do item no indice buscado
int posicaoNaLista(Lista* lista, int indice){
    if (lista->primeiroItem == NULL || indice >= tamanhoDaLista(lista) || indice < 0)
        return -1;
    ListaItem* aux = lista->primeiroItem;
    for (int i = 0; i < indice; i++) aux = aux->proximoItem;
    return aux->valor;
}
...
void limparLista(Lista* lista){
    ...
    while (lista->primeiroItem != NULL) removerPrimeiroItemDaLista(lista);
}
